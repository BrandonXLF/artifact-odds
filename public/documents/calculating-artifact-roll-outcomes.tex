...
\begin{itemize}[]
	...
	\item $InitialValueOutcomes$ is the number of possible roll outcomes.
...
As there are 4 stats to choose from each time, and since there are 4 roll values a roll can have (70\%, 80\%, 90\%, and 100\%), all these are exponents with a base of 4. Let $n$ be the number of rolls.
\begin{itemize}[]
	\item $InitialValueOutcomes = 4^4$ - There are 4 initial substats
	\item $RollOutcomes = 4^n$ - There are $n$ sub-stats to roll
	\item $RollValueOutcomes = 4^n$ - $n$ sub-stats where rolled, and need a value assigned
\end{itemize}
...
But instead, they can be groupped as so: 2 (1x), 1.5 (2x), 1 (3x), 0.5 (2x), 0 (1x). This leaves only 5 outcomes to bring to the next iteration. This is repeated for all of the $x$ iterations.
...
\begin{algorithm}[H]
	...
	\Return $Sum \div (\sum_{valid} p \times (4^n)^2(4^4))$
\end{algorithm}
